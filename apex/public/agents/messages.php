    ...
    <table>
      ...
      <?php while($message = db_fetch_assoc($message_result)) { ?>
        <?php
          ...
          //$recip_result = find_agent_by_id($message['recipient_id']);
          //$recip = db_fetch_assoc($recip_result);

          $string = $message['cipher_text'];
          $signature = $message['signature'];

          $message_text = $message['cipher_text'];
          $validity = verify_signature($message['cipher_text'], $signature, $sender['public_key']);
          $validity_text = $validity === 1 ? 'Valid' : 'Not valid';
          if($current_user['id'] == $agent['id']){
            $message_text = pkey_decrypt($string, $agent['private_key']);
          }
